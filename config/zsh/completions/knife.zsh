#compdef knife
# https://github.com/ohmyzsh/ohmyzsh/blob/master/plugins/knife/_knife

# You can override the path to knife.rb and your cookbooks by setting
# KNIFE_CONF_PATH=/$HOME/.cinc/knife.rb
KNIFE_COOKBOOK_PATH=/$HOME/Documents/chef/cookbooks
...
_chef_cookbooks_local() {
  if [ $KNIFE_RELATIVE_PATH ]; then
    ...
    local knife_rb=${KNIFE_CONF_PATH:-${HOME}/.cinc/knife.rb}
    if [ -f ./.cinc/knife.rb ]; then
      knife_rb="./.cinc/knife.rb"
    fi
...
_chef_root() {
  directory="$PWD"
  while [ $directory != '/' ]; do
    test -e "$directory/.cinc" && echo "$directory" && return
    directory="${directory:h}"
  done
}

if [ "$funcstack[1]" = "_knife" ]; then
    _knife "$@"
else
